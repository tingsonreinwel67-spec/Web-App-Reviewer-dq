-- Seed data generated from Traditional and VUL exam content
-- Run this after migrations 1-3 have been applied
BEGIN;
-- Questions + Choices
...
FROM new_question,
    (
        VALUES (
            ...
            ('The policy will automatically terminate', false),
            (
...
FROM new_question,
    (
        VALUES ('Buys a new plan altogether', false),
            (
...
FROM new_question,
    (
        VALUES ('The level of first year commission', false),
            ('Agent''s service oriented attitude', false),
            ('Pressure selling', true),
            ('The use of effective needs selling', false)
    ) AS v(text, is_correct);
WITH new_question AS (
    INSERT INTO questions (exam_type, category, text)
    VALUES (
...
FROM new_question,
    (
        VALUES (
            ...
            ('Void from the beginning', false),
            (
                'Voidable by the insurer if it has been in force less than 2 years',
                false
            ),
            ('Valid unless the insurer can prove fraud', true)
    ) AS v(text, is_correct);
WITH new_question AS (
    INSERT INTO questions (exam_type, category, text)
    VALUES (
...
FROM new_question,
    (
        VALUES (
            ...
            ('An individual on his own life', false),
            ('An individual on the life of his spouse', false),
            (
...
FROM new_question,
    (
        VALUES ('Renews a term life policy', false),
            (
...
FROM new_question,
    (
        VALUES ('Participating Endowment', false),
            ('Variable life policies', false),
            ('Participating whole life', true),
            ('Annuities', false)
    ) AS v(text, is_correct);
WITH new_question AS (
    INSERT INTO questions (exam_type, category, text)
...
WITH new_question AS (
    INSERT INTO questions (exam_type, category, text)
    VALUES ('VUL', 'General', 'A unit trust is ___________.')
    RETURNING id
)
INSERT INTO choices (question_id, text, is_correct)
SELECT id,
...
FROM new_question,
    (
        VALUES ('Php. 432,000.00', false),
            ('Php. 420,069.20', false),
            ('Php. 401,107.58', true),
            ('Php. 412,500.00', false)
    ) AS v(text, is_correct);
WITH new_question AS (
    INSERT INTO questions (exam_type, category, text)
    VALUES (
...
FROM new_question,
    (
        VALUES ('It has a high yield potential', false),
            (
...
-- Flashcards (randomized subset: front = question, back = correct answer)
...
-- Memorization (randomized subset, multiple choice)
WITH new_memo AS (
