...
export default function MemorizationPage() {
  ...
  const complete = (finalWrong = wrong) => {
    ...
    progress[type].memorize = Math.max(
      progress[type].memorize,
      100 - Math.round((finalWrong.length / questions.length) * 100),
    );
  ...
  const answer = (choiceId: string) => {
    ...
    advanceTimer.current = window.setTimeout(() => {
      answering.current = false;
      if (index === questions.length - 1) complete(updatedWrong);
      else {
        setIndex(index + 1);
        setSelected(null);
        setChecked(false);
  ...
  if (finished)
    return (
      <main className="mx-auto flex max-w-2xl flex-col gap-5 px-5 pb-10 pt-10">
        <Result correct={questions.length - wrong.length} wrong={wrong.length} onTryAgain={restart} onRedoMistakes={redoMistakes} />
      </main>
    );
  if (!question)
    return (
