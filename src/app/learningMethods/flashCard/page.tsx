...
export default function FlashCardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/questions?exam_type=${encodeURIComponent(type)}`)
      ...
      .then((items) => { if (active) { setQuestions(shuffled(items)); setIndex(0); setRevealed(false); setFinished(false); } })
  ...
  const scramble = () => { setQuestions((current) => shuffled(current)); setIndex(0); setRevealed(false); setFinished(false); };
  const complete = () => {
