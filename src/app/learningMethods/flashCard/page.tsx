...
function splitQuestion(text: string) {
  ...
  const starts = matches.map((match) => (match.index ?? 0) + match[0].indexOf(match[1]));
  return {
    prompt: text.slice(0, starts[0]).trim(),
    statements: starts.map((start, index) => text.slice(start, starts[index + 1]).trim()),
  };
}
...
function FlashCardContent() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-col gap-5 px-5 pb-10">
      ...
      {feedback ? (
        <section
          className={`flex min-h-[22rem] items-center justify-center rounded-3xl border p-6 shadow-sm ${feedback === "know" ? "border-emerald-500 bg-emerald-500/15 text-emerald-600 dark:text-emerald-300" : "border-rose-500 bg-rose-500/15 text-rose-600 dark:text-rose-300"}`}
        >
        ...
        >
          ...
          >
            <span className="col-start-1 row-start-1 flex min-h-[22rem] flex-col rounded-3xl border border-border bg-card p-6 shadow-sm [backface-visibility:hidden]">
            ...
            {questionParts && questionParts.statements.length > 0 && (
              <span className="mt-5 flex flex-col gap-2">
                {questionParts.statements.map((statement) => (
                  <span key={statement} className="rounded-xl bg-muted/40 px-4 py-3 text-sm font-normal leading-6">
                    {statement}
                  </span>
                ))}
              </span>
            ...
            <span className="col-start-1 row-start-1 flex min-h-[22rem] flex-col rounded-3xl border border-primary/30 bg-card p-6 shadow-sm [backface-visibility:hidden] [transform:rotateY(180deg)]">
