          ...
          <div className="mt-5 flex flex-col gap-2">
            ...
            {visibleTerms.length === 0 ? (
              ...
              visibleTerms.map((item) => (
                <div
                  key={item}
                  className="flex min-h-11 items-center justify-between gap-3 rounded-xl border border-border px-3 py-2.5 text-sm"
                >
                  <span className="min-w-0 wrap-anywhere">{item}</span>
                  <button
                    onClick={() => removeTerm(item)}
                    aria-label={`Remove ${item}`}
                    className="-mr-1 flex size-9 shrink-0 items-center justify-center text-muted-foreground hover:text-foreground"
                  >
